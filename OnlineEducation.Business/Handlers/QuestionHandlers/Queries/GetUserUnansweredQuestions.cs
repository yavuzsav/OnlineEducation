...
{
    public class GetUserUnansweredQuestions
    {
        ...
        public class Handler : IRequestHandler<Query, Pagination<QuestionDto>>
        {
            ...
            public async Task<Pagination<QuestionDto>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var userUnansweredQuestionsSpecification =
                    new UserUnansweredQuestionsSpecification(userId, request.PaginationParams);

                var questions =
                    await questionRepository.ListWithSpecificationAsync(userUnansweredQuestionsSpecification);

                var count = await questionRepository.CountAsync(new UserUnansweredQuestionsSpecification(userId));
